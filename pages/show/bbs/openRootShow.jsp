    ...
    <center>
      ...
      <html:link linkName="top"/>
        ...
        <table border="1" width="99%" cellspacing="0" cellpadding="0" bordercolor="#E3E3E3" bordercolorlight="#E0E0E0" bordercolordark="white" rules="all" style="margin-top:8px;word-break:break-all">
          <tr height="30"><td colspan="3" background="images/index/classT.jpg" style="text-indent:5px"><b><font color="white">■ 显示根帖 &nbsp;■ 悬赏积分：<bean:write name="bbsRootSingle" property="bbsjf" filter="false"/></font></b></td></tr>
          <tr height="30" bgcolor="#F0F0F0">
              <td style="text-indent:5px" width="27%">★ 楼主</td>
              <c:forEach var="row" items="${result3.rows}">
                    <c:if test="${sessionScope.logoner.userName==sessionScope.bbsRootSingle.bbsSender||row.user_able==2}">
                    <td colspan="2">【主题】<textarea id="tit" rows="1" onchange="up1()">${sessionScope.bbsRootSingle.bbsTitle}</textarea></td>
<%--                <bean:write name="bbsRootSingle" property="bbsTitle" filter="false"/>--%>
                    </c:if>
                    <c:if test="${sessionScope.logoner.userName!=sessionScope.bbsRootSingle.bbsSender&&row.user_able!=2}">
                      <td colspan="2">【主题】${sessionScope.bbsRootSingle.bbsTitle}</td>
                      <%--              <bean:write name="bbsRootSingle" property="bbsTitle" filter="false"/>--%>
                    </c:if>
              </c:forEach>
          ...
          <tr bgcolor="#F9F9F9">
              ...
              <td rowspan="3" align="center" valign="top">
                  <table width="95%" height="180" border="0" cellspacing="0" cellpadding="0" style="margin-top:3px">
                      ...
                      <!-- “返回”超链接 -->
                      <tr height="30"><td colspan="2" style="text-indent:70px"><a href="${sessionScope.servletPath}?method=${sessionScope.method}">[返回]</a></td></tr>
                  </table>
              </td>
              <!-- 根帖信息 -->
...
<%--          <bean:write name="bbsRootSingle" property="bbsContent" scope="session" filter="false"/>--%>
          ...
          <tr height="30" bgcolor="#F9F9F9">
              <td align="right" colspan="2">
                  <c:forEach var="row" items="${result3.rows}">
                  <c:if test="${sessionScope.logoner.userName==sessionScope.bbsRootSingle.bbsSender||row.user_able==2}">
                      ◆<a href="needLogin/firstBbs.do?method=mcon&bbsId=${sessionScope.bbsRootSingle.bbsId}&bbsSender=${sessionScope.bbsRootSingle.bbsSender}&bbscon=${sessionScope.bbsRootSingle.bbsContent}&bbstit=${sessionScope.bbsRootSingle.bbsTitle}" title="帖子所属者/楼主/管理员操作" id="a">修改帖子</a>
                  </c:if>
                  </c:forEach>
<%--                  ◆<a href="needLogin/deleteRoot.do?method=mcon&bbsId=${sessionScope.bbsRootSingle.bbsId}&bbsSender=${sessionScope.bbsRootSingle.bbsSender}&bbscon=${sessionScope.bbsRootSingle.bbsContent}&bbstit=${sessionScope.bbsRootSingle.bbsTitle}" >修改帖子</a>--%>
      ...
      <table border="1" width="99%" cellspacing="0" cellpadding="0" bordercolor="#E3E3E3" bordercolorlight="#E3E3E3" bordercolordark="white" style="margin-top:2px;word-break:break-all">
          <tr height="30"><td background="images/index/classT.jpg" style="text-indent:5px"><b><font color="white">■ 显示回复帖子</font></b></td></tr>
      </table>
      <!-- 不存在回复帖子列表 -->
          ...
          <logic:notEmpty name="answerbbslist" scope="session">
              ...
              <table border="1" width="99%" cellspacing="0" cellpadding="0" bordercolor="#E3E3E3" bordercolorlight="#E3E3E3" bordercolordark="white" rules="all" style="word-break:break-all">
                  <tr height="25" bgcolor="#F0F0F0">
                      <td style="text-indent:5px" width="27%">▲${(page.currentP-1)*page.perR+(idind+1)} 楼</td>
                                  ...
                                  <c:forEach var="val1" items="${row1}">
                                      <c:forEach var="row3" items="${result3.rows}">
                                          <c:if test="${sessionScope.logoner.userName==sessionScope.bbsRootSingle.bbsSender||row3.user_able==2}">
                                              ◆<a href="needLogin/deleteRoot.do?method=adjf&bbsId=${answerMap[answererName].id}&bbsAnswerId=${answerbbsSingle.bbsAnswerId}&addjf=${val1.value}">采纳回复</a>&nbsp;
                                          </c:if>
                                      </c:forEach>
                                  </c:forEach>
                                  </c:forEach>
      ...
      <html:form action="needLogin/answerBbs.do">
		  ...
		  <tr bgcolor="#F0F0F0" height="30"><td colspan="2" style="text-indent:5px" background="images/index/classT.jpg"><b><font color="white">■ 回复【<bean:write name="bbsRootSingle" property="bbsTitle" filter="false"/>】帖子</font></b></td></tr>
          <tr>
